...
work-item hierarchy) for the **MMO Catch Recording** external frontend, and hand it to the **Frontend
...
- **Story / Spike / Bug (leaf)** — each one is a unit of implementation and gets its **own** delivery
  ...
  - **Scenarios** (Gherkin `Given / When / Then`, where embedded in the description/ACs) — the behavioural
    spec and the end-to-end/integration test scenarios. Each scenario must become at least one
    route/integration or unit test in the delivered change.
