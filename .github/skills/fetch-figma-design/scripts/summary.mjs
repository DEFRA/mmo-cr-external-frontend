...
function frameRows(document) {
  return topFrames(document).map((frame) => {
    ...
    const size =
      box.width && box.height ? `${round(box.width)}×${round(box.height)}` : '—'
...
export function buildDesignMarkdown(record, { manifest, tokens } = {}) {
  ...
  for (const [id, entry] of Object.entries(nodes)) {
    ...
    lines.push(
      `## Node \`${id}\` — ${document.name ?? ''} (${document.type ?? ''})`,
      ''
    )
  ...
  if (tokens) {
    lines.push(
      '## Design tokens',
      '',
      `_Derived from: ${tokens.derivedFrom}. Full detail in \`assets/tokens.json\`._`,
      ''
    )
    ...
    if (tokens.typography?.length) {
      ...
      for (const t of tokens.typography) {
        lines.push(
          `| ${t.name ?? '—'} | ${t.fontFamily ?? '—'} | ${t.fontWeight ?? '—'} | ${t.fontSize ?? '—'} | ${t.lineHeightPx ?? '—'} |`
        )
      }
      lines.push('')
    }
...
export function buildSectionMarkdown(index) {
  ...
  for (const section of index.sections) {
    ...
    lines.push(
      '| # | frame | id | type | size | folder |',
      '| - | ----- | -- | ---- | ---- | ------ |'
    )
    frames.forEach((f, i) => {
      const size = f.width && f.height ? `${f.width}×${f.height}` : '—'
      lines.push(
        `| ${i + 1} | ${f.name ?? '—'} | \`${f.id}\` | ${f.type ?? '—'} | ${size} | \`${f.dir}\` |`
      )
    })
    lines.push('')
  }
