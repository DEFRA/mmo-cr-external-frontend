...
function parseApiBase(raw) {
  ...
  try {
    ...
    throw new SafeError('FIGMA_API_BASE is not a valid URL.', {
...
export function loadConfig(rawEnv = process.env) {
  ...
  return Object.freeze({
    ...
    assetHostAllowlist: Object.freeze(
      assetHostAllowlist.length
        ? assetHostAllowlist
        : DEFAULTS.assetHostAllowlist
    ),
    token: requireEnv(env, 'FIGMA_PAT'),
    imageFormats: Object.freeze(
      parseFormats(env.FIGMA_IMAGE_FORMATS, DEFAULTS.imageFormats)
    ),
