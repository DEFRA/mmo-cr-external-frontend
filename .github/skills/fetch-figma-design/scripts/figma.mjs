...
export function parseLocation(input, nodeFlag) {
  if (typeof input !== 'string' || input.trim() === '') {
    throw new SafeError('No Figma URL or file key provided.', {
      code: 'ERR_INPUT'
    })
  }
  ...
  if (raw.includes('figma.com') || raw.startsWith('http')) {
    ...
    try {
      ...
      throw new SafeError('Input is not a valid Figma URL.', {
        code: 'ERR_INPUT'
      })
    }
    ...
    const match = url.pathname.match(
      /\/(?:design|file|proto|board)\/([A-Za-z0-9_-]+)/
    )
  ...
  if (!fileKey || !FILE_KEY_RE.test(fileKey)) {
    throw new SafeError(
      'Could not parse a valid Figma file key from the input.',
      { code: 'ERR_INPUT' }
    )
  }
  if (nodeId && !NODE_ID_RE.test(nodeId)) {
    throw new SafeError('Parsed node id is not in a valid form.', {
      code: 'ERR_INPUT'
    })
  }
...
export function createFigmaClient(config) {
  ...
  async function get(path, { tolerate = [] } = {}) {
    ...
    if (res.status === 401 || res.status === 403) {
      throw new SafeError(
        'Figma denied access. If credentials appear wrong, ask the user to check the .env file.',
        { code: 'ERR_AUTH' }
      )
    }
    if (res.status === 404)
  ...
  return {
    ...
    async getNodes(key, ids, { depth, geometry } = {}) {
      ...
      const { body } = await get(
        `/v1/files/${enc(key)}/nodes?${params.join('&')}`
      )
      return body
    },
    ...
    async getLocalVariables(key) {
      const { status, body } = await get(
        `/v1/files/${enc(key)}/variables/local`,
        { tolerate: [403, 404] }
      )
