...
function assertGet(options) {
  ...
  if (method !== 'GET') {
    throw new SafeError(
      `Refusing non-GET request (${method}) — this skill is strictly read-only.`,
      { code: 'ERR_METHOD' }
    )
  }
}
...
export function assertAllowedHost(rawUrl, allowedHosts) {
  if (!Array.isArray(allowedHosts) || allowedHosts.length === 0) {
    throw new SafeError('Refusing request: no egress allowlist configured.', {
      code: 'ERR_EGRESS'
    })
  }
  let url
  try {
    ...
    throw new SafeError('Refusing to request an invalid URL.', {
      code: 'ERR_EGRESS'
    })
  }
...
export async function guardedFetch(
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    try {
      ...
      throw new SafeError(
        `Network request failed: ${redactString(String(err?.message ?? err))}`,
        { code: 'ERR_NETWORK' }
      )
    }
