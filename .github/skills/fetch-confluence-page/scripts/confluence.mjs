...
export function parsePageRef(input, { expectedHost } = {}) {
  if (typeof input !== 'string' || input.trim() === '') {
    throw new SafeError('No Confluence page URL or id provided.', {
      code: 'ERR_INPUT'
    })
  }
  ...
  try {
    ...
    throw new SafeError(
      'Input is neither a valid Confluence page id nor a URL.',
      { code: 'ERR_INPUT' }
    )
  }
  if (
    ...
    throw new SafeError(
      'The page URL does not match the configured Confluence site (CONFLUENCE_BASE_URL).',
      { code: 'ERR_INPUT' }
    )
  }
...
function authError(status) {
  if (status === 401 || status === 403) {
    throw new SafeError(
      'Confluence denied access. If credentials appear wrong, ask the user to check the .env file.',
      { code: 'ERR_AUTH' }
    )
  }
}
...
function rateLimitError(status) {
  if (status === 429) {
    throw new SafeError(
      'Confluence rate limit reached (429) — stopping early without retrying. Wait a while before re-running.',
      { code: 'ERR_RATE_LIMIT' }
    )
  }
}
...
function nextCursor(data) {
  ...
  const cursor = new URLSearchParams(next.slice(next.indexOf('?') + 1)).get(
    'cursor'
  )
  return cursor || null
}
...
export function createConfluenceClient(config) {
  ...
  function assertGet(options) {
    if (options?.method && options.method.toUpperCase() !== 'GET') {
      throw new SafeError(
        'Refusing a non-GET request: this skill is strictly read-only.',
        { code: 'ERR_READONLY' }
      )
    }
  }
  ...
  async function getAttachments(id) {
    ...
    return {
      attachments: attachments.slice(0, config.maxAttachments),
      truncated
    }
  }
