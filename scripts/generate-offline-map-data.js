...
const outputDirectory = path.join(
  rootDirectory,
  'src/client/public/offline-map'
)
const codesPath = path.join(
  rootDirectory,
  'src/server/common/data/offline-map-subrectangle-codes.js'
)
const subrectanglesPath = path.join(
  rootDirectory,
  'src/server/common/data/offline-map-subrectangles.js'
)
const portsPath = path.join(
  rootDirectory,
  'src/server/common/data/offline-map-ports.js'
)
...
export function coordinate(value) {
  ...
  const projectedLatitude =
    ...
    (180 / Math.PI)
  return validCoordinate(projectedLongitude, projectedLatitude)
    ? [projectedLongitude, projectedLatitude]
    : undefined
}

function ring(value) {
  const coordinates = Array.isArray(value) ? value.map(coordinate) : []
  return coordinates.length >= 3 && coordinates.every(Boolean)
    ? coordinates
    : undefined
}
...
function polygon(value) {
  ...
  return exterior
    ? { exterior, holes: value.slice(1).map(ring).filter(Boolean) }
    : undefined
}
...
export function overlapsSea(subrectanglePolygons, landPolygons) {
  ...
  const candidates = landPolygons.filter((land) =>
    intersects(rectangle, land.bounds)
  )
  return (
    !candidates.length ||
    sampleFractions.some((longitudeFraction) =>
      sampleFractions.some(
        (latitudeFraction) =>
          !candidates.some((land) =>
            pointInPolygon(
              [
                rectangle.minLongitude +
                  (rectangle.maxLongitude - rectangle.minLongitude) *
                    longitudeFraction,
                rectangle.minLatitude +
...
export async function generateOfflineMapData() {
  const [landCollection, subrectangleCollection, portCollection] =
    await Promise.all([
      collection('map.geojson'),
      collection('subrectangles.geojson'),
      collection('ports.geojson')
    ])
  const land = landCollection.features
    .map((feature) => polygons(feature.geometry))
    .filter((featurePolygons) => featurePolygons.length)
    .map((featurePolygons) => ({
      polygons: featurePolygons,
      bounds: bounds(featurePolygons)
    }))
  const landPolygons = land.flatMap((feature) =>
    feature.polygons.map((landPolygon) => ({
      ...landPolygon,
      bounds: bounds([landPolygon])
    }))
  )
  const codes = new Set()
    ...
    return [
      {
        ...
        bounds: rectangle,
        labelCoordinate: [
          (rectangle.minLongitude + rectangle.maxLongitude) / 2,
          (rectangle.minLatitude + rectangle.maxLatitude) / 2
        ],
        overlapsSea: overlapsSea(featurePolygons, landPolygons)
      }
    ]
  })
  const ports = portCollection.features.flatMap((feature) =>
    points(feature.geometry)
      .map((portCoordinate) => ({
        portCode: numericValue(feature.properties?.port_code),
        name: feature.properties?.port,
  ...
  // Land-only rectangles have no fishing area, so the server-side lookup/validation data must exclude them.
  const seaSubrectangles = subrectangles.filter(
  ...
  await Promise.all([
    ...
    writeFile(
      codesPath,
      `// Generated by scripts/generate-offline-map-data.js. Do not edit.\nexport const offlineMapSubrectangleCodes = new Set(${JSON.stringify(seaSubrectangles.map(({ subCode }) => subCode).sort())})\n`
    ),
    writeFile(
      subrectanglesPath,
      `// Generated by scripts/generate-offline-map-data.js. Do not edit.\nexport const offlineMapSubrectangles = new Map(${JSON.stringify(seaSubrectangles.map(({ subCode, labelCoordinate }) => [subCode, { coordinate: labelCoordinate, icesRectangle: subCode.slice(0, -1) }]))})\n`
    ),
    writeFile(
      portsPath,
  ...
  return {
    land: land.length,
    subrectangles: subrectangles.length,
    sea: seaSubrectangles.length,
    ports: ports.length
  }
}
...
if (
  ...
  generateOfflineMapData()
    .then((result) =>
      console.log(
        `Generated offline map data: ${result.land} land, ${result.subrectangles} subrectangles (${result.sea} sea-overlapping), ${result.ports} ports`
      )
    )
    .catch((error) => {
